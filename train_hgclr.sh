...
#SBATCH --output=logs/hgclr_out_%j.txt   # Standard output log (%j = job ID)
#SBATCH --error=logs/hgclr_err_%j.txt    # Standard error log
...
source $HOME/hgclr/venv/bin/activate
...
python $HOME/hgclr/train.py \
    --name 'hgclr_experiment_1' \
    --seed 1 \
    --data 'panet' \
